...
public class ProducerTest {
	...
	protected static class NoLoggingCollectionProducer extends
			CollectionProducer<Object> {

		public NoLoggingCollectionProducer(final Collection<Object> data,
				final ThreadingModel model, final PipeLink<Object> out) {
	...
	protected static class NopProducer extends Producer<Object> {
		...
		public NopProducer(final ThreadingModel threading,
				final PipeLink<Object> link) {
	...
	public void creatingNullSignalsFinished() {
		...
		final Producer producer = new NoLoggingCollectionProducer(Arrays
				.asList(first, null), new InlineThreadingModel(), outLink);
		assertFalse("Started out finished already", producer.isFinished());
		producer.execute();
		assertSame("Got different values for test production", first, outLink
				.get());
		assertFalse("Finished after first item", producer.isFinished());
		producer.execute();
		assertEquals("Got another element we didn't expect to see", 0, outLink
				.getQueueLength());
	...
	public void explodingAddSignalsIsFinished() {
		...
		final Producer producer = new NoLoggingCollectionProducer(Arrays
				.asList(first, second, third), new InlineThreadingModel(),
				outLink);
